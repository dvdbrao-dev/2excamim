...
still does not imply submission, amendment, cancellation, or completion semantics.
...
- `Weak`: the boundary is only partially supported, such as a decision with no fills yet or a fill
  with only an external `order_id`, or a fill whose order was never locally registered.
